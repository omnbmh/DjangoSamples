...
def api_key_get(params, request_path):
    ...
    params.update({'AccessKeyId': huobi.global_settings.ACCESS_KEY,
    ...
    params['Signature'] = createSign(params, method, host_name, request_path, huobi.global_settings.SECRET_KEY)
...
def api_key_post(params, request_path):
    method = 'POST'
    timestamp = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S')
    params_to_sign = {'AccessKeyId': huobi.global_settings.ACCESS_KEY,
                      'SignatureMethod': 'HmacSHA256',
                      'SignatureVersion': '2',
                      'Timestamp': timestamp}
    ...
    params_to_sign['Signature'] = createSign(params_to_sign, method, host_name, request_path,
                                             huobi.global_settings.SECRET_KEY)
